...
public class AgenciaTest {
    ...
    public void nao_deve_aceitar_valor_com_caractere_invalido_no_numero() {
        ...
        assertTrue(verificaErro(agencia, MensagensErro.STRING_INVALIDO));
    }
    ...
    public void deve_aceitar_valor_sem_caractere_invalido_no_numero() {
        ...
        assertFalse(verificaErro(agencia, MensagensErro.STRING_INVALIDO));
    }
    ...
    @Test
    public void deve_haver_construtor_publico_com_argumento_do_tipo_String_Banco() {
        try {
            Agencia.class.getDeclaredConstructor(String.class, Banco.class).newInstance("2244", banco);
        } 
    ...
    public void o_construtor_deve_alimentar_o_numero_da_nova_instancia() {
        try {
            assertThat(Agencia.class.getDeclaredConstructor(String.class, Banco.class).newInstance("2244", banco).getNumero(), is(equalTo("2244")));
        } 
        catch (InstantiationException | IllegalAccessException | IllegalArgumentException | InvocationTargetException | NoSuchMethodException | SecurityException e) {
            fail();
        }
    ...
    @Test
    public void o_construtor_deve_alimentar_o_banco_da_nova_instancia() {
        try {
            assertThat(Agencia.class.getDeclaredConstructor(String.class, Banco.class).newInstance("2244", banco).getBanco(), is(equalTo(banco)));
        } 
        catch (InstantiationException | IllegalAccessException | IllegalArgumentException | InvocationTargetException | NoSuchMethodException | SecurityException e) {
            fail();
        }
    ...
    public void metodo_toString_deve_gerar_representacao_do_objeto_em_json_com_todos_os_atributos_da_classe() {
        ...
        assertThat(agencia.toString().charAt(0), is(equalTo('{')));
        for (int i = 0; i < Agencia.class.getDeclaredFields().length; i++) {      
            formatoEsperado.append("\"").append(Agencia.class.getDeclaredFields()[i].getName()).append("\":");         
            assertTrue(agencia.toString().contains(formatoEsperado.toString()));
            formatoEsperado.delete(0, formatoEsperado.length());          
            formatoEsperado.append(",");
        }
        assertThat(agencia.toString().charAt(agencia.toString().length() - 1), is(equalTo('}')));
    }
