...
public class BairroTest {
    ...
    public void metodo_toString_deve_gerar_representacao_do_objeto_em_json_com_todos_os_atributos_da_classe() {
        ...
        assertThat(bairro.toString().charAt(0), is(equalTo('{')));
        for (int i = 0; i < Bairro.class.getDeclaredFields().length; i++) {      
            formatoEsperado.append("\"").append(Bairro.class.getDeclaredFields()[i].getName()).append("\":");         
            assertTrue(bairro.toString().contains(formatoEsperado.toString()));
            formatoEsperado.delete(0, formatoEsperado.length());          
            formatoEsperado.append(",");
        }
        assertThat(bairro.toString().charAt(bairro.toString().length() - 1), is(equalTo('}')));
    }
